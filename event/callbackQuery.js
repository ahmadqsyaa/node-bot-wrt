...
export const callbackQueryEvent = async (bot, query, chatId, messageId, data) => {
    ...
    case "dell-cntb":
        await clean('/etc/crontabs/root')
        contab = await execute('cat /etc/crontabs/root')
        await bot.editMessageText(`please send me the number of lines you want to delete in contab.\n\n<b>${contab}</b>\n\nexample: 2\nsend <b>exit</b> to exit or cancel`, options)
        bot.once('message', async (response) => {
            var mess = response.text;
            if (mess == "exit") return bot.sendMessage(chatId, 'exit ✅',{reply_to_message_id:messageId})
            var result = isnumber(Number(mess), async (number) => {
                var befor = await execute(`sed -n '${number}p' /etc/crontabs/root`)
                var hps = await execute(`sed -i '${number}d' /etc/crontabs/root`)
                await clean('/etc/crontabs/root')
                var contab = await execute('cat /etc/crontabs/root')
                if (!hps) return bot.sendMessage(chatId, `success delete <s>${befor}</s>\n<b>${contab}</b>`, {
                    "parse_mode": "html",
                    "reply_to_message_id": `${messageId}`
                })
            });
            if (result) {
                bot.sendMessage(chatId, result, {reply_to_message_id: messageId})
            }
        });
        break;
    ...
    case "dell-start":
        ...
        bot.once('message', async (response) => {
            ...
            if (mess == "exit") return bot.sendMessage(chatId, 'exit ✅',{reply_to_message_id: messageId})
            var result = isnumber(Number(mess), async (number) => {
                var befor = await execute(`sed -n '${number}p' /etc/rc.local`)
                var hps = await execute(`sed -i '${number}d' /etc/rc.local`)
                await clean('/etc/rc.local')
                startup = await execute('cat /etc/rc.local')
                if (!hps) return bot.sendMessage(chatId, `success delete <s>${befor}</s>\n<b>${startup}</b>`, {
                    "parse_mode": "html",
                    "reply_to_message_id": `${messageId}`
                })
		...
		if (data.includes("passwall") || data.includes("openclash")) {
			await bot.editMessageText("loading", options);
